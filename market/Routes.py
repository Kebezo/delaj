...
def market_page(specialty, city):
    ...
    if request.method == "GET":
        if city.lower() != 'v' and specialty.lower() != 'v':
            items = (
                db.session.query(Ordinacija, Zdravnik, Naslov, Kraj)
                    ...
                    .filter(Ordinacija.tip == specialty, Kraj.ime == city)
                    .all()
            )
            ...
            items = (
                db.session.query(Ordinacija, Zdravnik, Naslov, Kraj)
                    ...
                    .join(Naslov, Ordinacija.lokacija == Naslov.id)
                    .join(Kraj, Naslov.kraj == Kraj.id)
                    .filter(Ordinacija.tip == specialty)
                    .all()
            )
        elif city.lower() != 'v' and specialty.lower() == 'v':
            items = (
        ...
        return render_template('termin.html', specialty= specialty, city=city, items=items, purchase_form=select_form, owned_items=owned_items,
                               selling_form=selling_form, datum=datum, termini=termini, ordinacija=ordinacijaform,
                               user=user, appointments=appointments, current_time=current_time, danes=danes,
                              )
...
def zdravnik_required(func):
    ...
    def decorated_view(*args, **kwargs):
        if not current_user.is_authenticated or current_user.role != "zdravnik":
...
def user_page():
    ...
    if appointments:
        ...
        for appointment in appointments:
            message += f"\n{appointment[0].datum}: {appointment[1].naziv}"
...
def zdravnik_page():
    ...
    ordinacija_id = past_appointments[2][2].id
    ...
    return render_template("zdravnik.html", user=user, appointments=appointments, past_appointments=past_appointments,
                           ordinacija_id=ordinacija_id, ponudbe=ponudbe)
...
def izberite_ordinacija():
    ...
    types = db.session.query(Ordinacija.tip).distinct().all()
...
def add_ponudba(id):
    if request.method == 'POST':
        ...
        ponudba = Ponudba(naziv=naziv, dolzina=dolzina, ordinacija=id)
...
def is_date_free(ponudba_id, date, duration, working_hours_start, working_hours_end):
    appointments = (
        db.session.query(Termin, Ponudba, Ordinacija)
            .join(Ponudba, Termin.punudba == Ponudba.id)
            .join(Ordinacija, Ordinacija.id == Ponudba.ordinacija)
            .filter(Termin.datum == date)
            .all()
    )
    if not appointments:
        return True
...
def get_free_slots():
    ...
    availability = {}
    for date in dates:
        availability[str(date)] = is_date_free(ponudba_id, date, duration, working_hours_start, working_hours_end)
    return availability
